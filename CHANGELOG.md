...
- **`tool_result.is_error` is no longer silently dropped when translating to OpenAI's wire format.** Anthropic's `tool_result` carries a structural `is_error` flag; OpenAI `tool` messages have no equivalent field, and both adapters previously ignored it entirely. Failed results are now prefixed with `[tool error] ` in the translated content. Note: as of this release, real MCP tool failures are already flattened into `{"error": ...}` text by `mcp/client.ts` before this flag is ever set, so this prefix currently only fires for the internal aux-compaction sub-loop's synthetic "Unknown tool" result — it future-proofs the translators for any `is_error: true` block a host constructs directly, but isn't yet wired to the live MCP-failure path. See the tracked follow-up to thread a real `is_error` flag through `mcp/client.ts` / `tools/registry.ts` if MCP failures should read as failures to these models.
